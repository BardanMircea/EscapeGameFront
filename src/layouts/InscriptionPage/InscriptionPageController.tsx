...
const InscriptionPageController = () => {
    ...
    const handleSubmit = async (event : any) => {
        ...
        const utilisateur : UtilisateurRequestModel = new UtilisateurRequestModel(nom, prenom, email, mdp, naissance)
        ...
        if(response.status === 409) {
            ...
            return
        } else if(!response.ok){                        // else, if the received status is not the 200(ok) status, throw a generic error 
