...
const ReservationPageController = () => {
  ...
  const { salle_id, salle_nom, salle_capacite, salle_img, jour, creneau } =
    useParams();

  // decode the encoded image
  const decodedImg = salle_img
    ? salle_img
  ...
  function changeNumberOfParticipants(event: any): void {
    ...
    for (let i = 0; i < parseInt(value); i++) {
      participantsArr.push(
        new ParticipantModel(participantsArr.length, "", "", "", "")
      );
    }
  ...
  async function submitReservation(event: any) {
    ...
    const submitParticipantsResponse = await fetch(
  ...
  function updateParticipantPrenom(
    id: number,
    event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ): void {
    const participantsCopy = [...participants];
  ...
  function updateParticipantNaissance(
    id: number,
    event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ): void {
    const participantsCopy = [...participants];
  ...
  return (
    <>
      <h1>
        {<span style={{ color: "rgb(124, 31, 124)" }}>{salle_nom}</span>} Escape
        Room
      </h1>
      <div className="card-details-container">
        <Card sx={{ width: 1000, maxWidth: 1200, maxHeight: 10000, mb: 5 }}>
          ...
          <CardContent>
            ...
            <br />
            <p style={{ fontSize: 20 }}>
              Le nombre de participants doit entre compris entre{" "}
              <span style={{ color: "green" }}>
                {" "}
                <i>{salle_capacite}</i>
              </span>
            ...
            {showInputsForm && (
              <>
                ...
                {participants.map((participant) => (
                  <div
                    className="participants-input-container"
                    key={participant._id}
                  >
                    <TextField
                      sx={{ mb: 1 }}
                      ...
                      onChange={(event) =>
                        updateParticipantNom(participant._id, event)
                      }
                    />
                    <TextField
                      sx={{ mb: 1 }}
                      ...
                      onChange={(event) =>
                        updateParticipantPrenom(participant._id, event)
                      }
                    />
                    <TextField
                      sx={{ mb: 1 }}
                      ...
                      variant="outlined"
                      value={participant.naissance}
                      onChange={(event) =>
                        updateParticipantNaissance(participant._id, event)
                      }
                    />
                  </div>
                ))}
